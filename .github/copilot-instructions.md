...
- Pausing an active round now persists a gameplay snapshot in `localStorage`, and app boot restores that snapshot back into a paused round before showing the intro leaderboard. If you change pause, startup, or runtime state structures, keep the saved-game serializer and restorer in sync.
